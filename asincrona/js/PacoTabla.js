async function funcion() {
  //* Si hace exito.
  let response = await fetch("PacoApiPost.json"); //todo Pagina donde vamos a cargar todos los datos, es un json.
  ...
  try {
    user.forEach((item) => {
      ...
      crearDiv.innerHTML = `<img src="${item.imagen}"></img><h2>Post del id ${item.user_id}</h2> <p> <strong>Titulo:</strong> ${item.titulo}</p><p><strong>Body: </strong>${item.contenido}</p> <p>Estado: <span>${item.estado}</span></p>`; //? Agregamos el post del id , el titulo de cada uno , el contenido, el estado y demas
      ...
      console.log(primerElementoHijoDivs); //? Esto saca el primer hijo, basicamente el post del id tal....
